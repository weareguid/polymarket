"""
Extract daily probability time series from quant.parquet for new target markets v5.

Phase: Batch 5 — ~200 new markets not in batches 1, 2, 3, or 4.

Strategy: Continued diversification, filling gaps in 2022-2024 data.
  - Categories: geopolitical, macro, us_politics, defense, energy, commodities, corporate, ai
  - Volume threshold: >= 75,000 USD (lower than v4 to increase variety)
  - End date window: 2022-01-01 → 2026-01-01
  - Priority: ALL available 2022-2023 markets (only 46 total), then 2024 backfill
  - Category caps tuned to available pool per category

Outputs:
  - data/historical/price_series_historical/<condition_id>.parquet  (shared dir)
  - data/historical/price_series_targets_v5.csv                     (new targets)
  - data/historical/price_series_historical_index_v5.csv            (new index)
"""
import pandas as pd
import numpy as np
...
TARGETS_V5 = "data/historical/price_series_targets_v5.csv"
INDEX_V5   = "data/historical/price_series_historical_index_v5.csv"
...
t4 = pd.read_csv("data/historical/price_series_targets_v4.csv")

already_targeted = set(
    t1["id"].tolist() + t2["id"].tolist() + t3["id"].tolist() + t4["id"].tolist()
)
print(f"Already targeted (batches 1+2+3+4): {len(already_targeted)} markets")
...
print(f"Total candidates (vol>=75k, date in range): {len(candidates)}")
print()

# ---------------------------------------------------------------------------
# Category caps tuned to available pool — target ~200 total
# All 2022-2023 markets are included first (only 46 available across all cats)
# Then fill with top-by-volume 2024 markets
# ---------------------------------------------------------------------------
CATEGORY_CAPS = {
    "us_politics":  50,   # large pool (2814 candidates), good variety
    "geopolitical": 45,   # large pool (818), rich international events
    "macro":        30,   # 223 candidates, all macro interest rates/inflation
    "corporate":    25,   # 246 candidates, earnings + M&A
    "ai":           25,   # 226 candidates, AI race markets
    "defense":      10,   # 47 candidates, cap near pool size
    "commodities":  10,   # 36 candidates
    "energy":        3,   # only 3 candidates total
}
# Total cap = 198
...
new_targets[save_cols].to_csv(TARGETS_V5, index=False)
print(f"Targets saved to: {TARGETS_V5}")
print()
...
idx_df.to_csv(INDEX_V5, index=False)
